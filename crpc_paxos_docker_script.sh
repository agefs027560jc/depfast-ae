        ...
        for run_id in {1..3}; do
            ...
            for process_id in $(seq 1 $server_count); do
                ...
                if [ "$process_id" -eq 1 ]; then
                    ...
                    build/deptran_server -f config/occ_paxos.yml -f "$config_filename" -f config/rw.yml -f config/concurrent_1.yml -P p$process_id -v $v_value -d 60 > "$output_node" &
                else
                    sudo docker exec -d "$container_name" \
                    build/deptran_server -f config/occ_paxos.yml -f "$config_filename" -f config/rw.yml -f config/concurrent_1.yml -P p$process_id -v $v_value -d 60 &
                fi
            ...
            echo "Monitoring bandwidth for 60 seconds on nano_1..."
            sleep 5
