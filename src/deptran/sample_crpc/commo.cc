...
shared_ptr<SampleCrpcQuorumEvent> SampleCrpcCommo::broadcast_add(parid_t par_id,
  ...
  uint64_t crpc_id = ++crpc_id_counter;
  //Log_info("*** crpc_id is: %d", crpc_id); // verify it's never the same
  uint64_t sz = Config::GetConfig()->sites_.size();
  ...
  for (auto& p : proxies) {
    ...
    fuattr.callback = [this, e, n, ip, crpc_id, sz] (Future* fu) {
      ...
      //Log_info("Tracepath: %d, %d", sz - 1, crpc_id);
      int64_t accept = 0;
    ...
    auto f = proxy->async_BroadcastAdd(crpc_id, sz - 1, value1, value2, fuattr); // #profile - 1.36%
    Future::safe_release(f);
    sz--;
  }
  // verify(!e->IsReady());
